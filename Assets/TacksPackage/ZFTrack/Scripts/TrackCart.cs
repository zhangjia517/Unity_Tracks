...
public class TrackCart : MonoBehaviour {
	 ...
	 * use SwitchCurrentSectionTo() to attach the cart to a track.
	 */
	[SerializeField]
	 ...
	 */
	public float clearingDistance = 1;
	...
	ConfigurableJoint trackJoint, guideJoint;
...
//		if (!guide) StartCoroutine(setupGuide());
		if (!guide) SetupGuide();
...
//		Transform cartTransform = guide.transform;
		...
		SimpleTransform trackTangent = track.transform * track.Curve.GetPointAt(pos);

		if (cartReversed) {
			trackTangent.AboutFace();
		}

		if (DRAW_DEBUG && _currentTrack) {
		...
		//now do acceleration/decel
		PushThings(trackTangent);
	}

	protected void PushThings(SimpleTransform trackTangent) {
		if (!_currentTrack) return;

		//apply acceleration/deceleration as per the track's spec


		float forwardness = Vector3.Dot(GetComponent<Rigidbody>().velocity, trackTangent.rotation * Vector3.forward);
		...
		if (brakesDirection != 0 && speed > _currentTrack.brakes.targetSpeed) {
			...
			if (prorateForces) {
				float bestForce = getOptimumForce(speed, _currentTrack.brakes.targetSpeed, GetComponent<Rigidbody>().mass);
				//Debug.Log("Best braking force: " + bestForce + " max breaking force " + force);
				//(force and bestForce should be negative, hence greater than)
				if (bestForce > force) force = bestForce;
			}
			GetComponent<Rigidbody>().AddForce(GetComponent<Rigidbody>().velocity.normalized * force);
			...
			if (prorateForces) {
				float bestForce = getOptimumForce(speed, _currentTrack.acceleration.targetSpeed, GetComponent<Rigidbody>().mass);
			...
			GetComponent<Rigidbody>().AddForce(pushDirection * force);
		}
	...
	protected void SetupGuide() {
		...
		rb.drag = 0;
		rb.angularDrag = 0;
		//todo: steal some ratio of mass from the cart so the guide doesn't make it heavier
		...
		guideJoint.connectedBody = _currentTrack ? _currentTrack.GetComponent<Rigidbody>() : null;
	...
	/**
	 * Resets the internal target position in the joint back to what it should be.
	 * (Continually changing the joint leads to rounding errors in the target.)
	 *
	 * Too bad we don't have a simple API property for this :-(
	 */
	protected void resetJoint(SimpleTransform correctTarget = null) {
//		Debug.Log("Reset joint");
//
//		if (trackJoint) {
//			Object.Destroy(trackJoint);
//		}
//
//		//move to correct position
////		SimpleTransform prevLoc = new SimpleTransform(transform);
//		if (correctTarget) {
//			transform.position = correctTarget.position;
//			transform.rotation = correctTarget.rotation;
//		}
//
//		//make trackJoint
//		trackJoint = gameObject.AddComponent<ConfigurableJoint>();
//
//		trackJoint.xMotion = ConfigurableJointMotion.Free;
//		trackJoint.yMotion = ConfigurableJointMotion.Locked;
//		trackJoint.zMotion = ConfigurableJointMotion.Locked;
//		trackJoint.angularXMotion = ConfigurableJointMotion.Locked;
//		trackJoint.angularYMotion = ConfigurableJointMotion.Locked;
//		trackJoint.angularZMotion = ConfigurableJointMotion.Locked;
//
//		trackJoint.anchor = Vector3.zero;
//		trackJoint.configuredInWorldSpace = true;
//
...
////		if (correctTarget) {
////			transform.position = prevLoc.position;
////			transform.rotation = prevLoc.rotation;
////		}
	}
