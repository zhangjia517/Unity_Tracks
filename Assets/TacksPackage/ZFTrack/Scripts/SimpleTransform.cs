...
public class SimpleTransform {
	...
	public SimpleTransform MakeValid() {
		if (rotation.w == 0 && rotation.x == 0 && rotation.y == 0 && rotation.z == 0)
			rotation.w = 1;
		if (double.IsNaN(position.x) || double.IsInfinity(position.x)) position.x = 0;
		if (double.IsNaN(position.y) || double.IsInfinity(position.y)) position.y = 0;
		if (double.IsNaN(position.z) || double.IsInfinity(position.z)) position.z = 0;
