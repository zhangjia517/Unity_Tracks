...
public class TrackMeshGenerator {
	...
	public void GenerateMesh(Mesh mesh, Track track) {
		...
		if (capMesh != null && (!track.NextTrack || !track.PrevTrack)) {
			...
			for (int triIdx = 0; triIdx < capMesh.tris.Count; triIdx += 3) {
				...
				if (pos0.z < 0) {
					...
					if (track.PrevTrack) continue;//can't see this cap
					offset = .5f;
					trans = firstStep;
				} else {
					//this tri falls on the end side
					if (track.NextTrack) continue;//can't see this cap
					offset = -.5f;
					trans = lastStep;
				}
		...
		if (tieMesh) {
			...
			for (int i = 0; i < numTies; i++) {
				var tiePos = track.Curve.GetPointAt(i / numTies + offset);
				int pInitial = allVerts.Count;
