...
public class Track : MonoBehaviour {
	...
	public enum EndOfLineBehavior {
		...
		//EndStop,not implemented //stop as if we hit a wall
	}
	 ...
	 */
	public TrackCurve.CurveType curveType = TrackCurve.CurveType.Hermite;

	public float curveStartStrength = 1.4142135f, curveEndStrength = 1.4142135f;
...
#if UNITY_EDITOR
		if (Application.isEditor && !Application.isPlaying && !Application.isLoadingLevel) {
	...
	public SimpleTransform NearestPoint(Vector3 pos) {
		...
		return Curve.GetPointAt(nearestFraction);
	}
	...
	protected void GenerateCollider() {
		if (GetComponent<Rigidbody>()) {
			// Unity doesn't have concave moving hulls, so this isn't a simple problem :-(
			// Later, we'll probably make it work anyway by making a number of convex hulls from the pieces of track
			// (assuming the track itself is convex or can be split into convex pieces), then group them all together.
			// Then we will pray that it won't kill performance nearly as badly as it sounds like it will.
			Debug.LogWarning("Track is marked tangible and has a rigidbody. This isn't supported");
		} else {
			//for static geometry, we can just use a concave mesh
