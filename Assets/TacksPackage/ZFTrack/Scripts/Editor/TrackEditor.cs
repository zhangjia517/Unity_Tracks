...
public class TrackEditor : Editor {
	...
	protected void RenderMultiSelectNavOptions() {
		...
		{ 
			...
			if (GUILayout.Button("Merge")) MergeTracks(selectionOrder);
			GUI.enabled = true;
		}
		GUILayout.EndHorizontal();
	...
	public void SplitTrack() {
		...
		//Turn this on, I bet they're about to us it.
	 ...
	 */
	protected Track AddTrack(Track track, bool prevTrack) {
		//make a new track!
	...
	protected Object[] GetObjectsInvolvedWithTrack(Track track) {
	...
	public void MergeTracks(List<Track> tracks) {
		...
		foreach (var track in tracks.Skip(1)) {
			//FIXYOU: This causes Unity to "MissingReferenceException: The object of type 'MeshCollider' has been destroyed but you are still trying to access it."
			//inside its editor code, but I can't fix that as I can't adjust it :-(
